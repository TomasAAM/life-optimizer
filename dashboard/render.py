...
def _source_health_html(source_status: pd.DataFrame) -> str:
    """Render current source outcomes without exposing provider error text.

    Parameters
    ----------
    source_status : pandas.DataFrame
        Rows from :func:`dashboard.query.fetch_ingestion_status`.

    Returns
    -------
    str
        Compact source-health markup. Degraded, failed, or skipped stages are
        emphasized; successful stages remain a quiet freshness line.
    """
    if source_status.empty:
        return (
            "<div class='source-health source-health-warn'>Source freshness is "
            "unavailable.</div>"
        )

    labels = {
        "garmin_auth": "Garmin authentication",
        "garmin_wellness": "Garmin wellness",
        "garmin_activities": "Garmin activities",
        "garmin_exercise_sets": "Garmin exercise sets",
        "body_composition": "Body composition",
    }
    items: list[str] = []
    has_problem = False
    order = {source: index for index, source in enumerate(labels)}
    rows = sorted(
        source_status.to_dict(orient="records"),
        key=lambda row: (
            order.get(str(row.get("source")), len(labels)),
            str(row.get("source")),
        ),
    )
    for row in rows:
        ...
        last_success = pd.to_datetime(
            row.get("last_success_at"), errors="coerce", utc=True
        )
        freshness = (
            last_success.strftime("%d %b %H:%M UTC")
        ...
        if status != "success":
            has_problem = True
        items.append(
            f"<span><strong>{escape(label)}</strong>: {escape(status)} "
            f"(last success {escape(freshness)})</span>"
        )

    css_class = "source-health-warn" if has_problem else "source-health-ok"
    lead = "Source attention required" if has_problem else "Sources refreshed"
    return (
        f"<div class='source-health {css_class}'><b>{lead}:</b> "
        + " · ".join(items)
        + "</div>"
    )
